...
- E1.31 sender design: hand-rolled packet framing recommended (fixed ~638-byte layout, byte-asserted unit tests per WS-4.4) so the send-on-change wake is fully ours; `sacn` library noted as the alternative. Hardware validation stays gated on universe-box verification (D-017, transport doc §6) — code lands behind the null default.

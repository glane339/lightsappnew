...
that frames and transmits real sACN. A fresh install defaults to ``dmx.transport =
"e131"`` and ``ledfx.enabled = true``. The universe-box IP still lives only in local
``config.json`` (never in the repo). Tests force a silent transport. Manual tap remains as
...
- `backend/` — Python backend
  ...
  - `ledfx/` — LEDfx HTTP client and scene sync (on by default)
